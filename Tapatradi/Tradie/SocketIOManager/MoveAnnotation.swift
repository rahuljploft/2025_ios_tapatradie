...
class MoveAnnotation:NSObject {
    var loc_receiver: CLLocation!
    var loc_provider: CLLocation!
    
    var map: MKMapView?
    var ano: MyAnnotation? = nil
    ...
    func addNewLocation (_ loc:CLLocationCoordinate2D) {
        ...
        let ob = CalculatePath(loc_last: loc_old!, loc_new: loc)
    ...
    func drawRoute (_ loc: CLLocation) {
        let ob = PolilineObject(GOOGLE_KEY)
        ob.ll2 = loc_provider!
        ob.ll1 = loc//CLLocation(latitude: point.latitude, longitude: point.longitude)
        ob.aii = false
        ob.popup = false
        ob.prnt = false
        ob.key = GOOGLE_KEY
            ...
            while (true) {
                ...
                if self.ano != nil {
                    if self.maPaths.count > 0 {
                        let ob = self.maPaths[0] as? CalculatePathReturn
                        ...
                        if ob != nil {
                            if ob?.points != nil {
                                if (ob?.points?.count)! > 0 {
                                    ...
                                    let index = (ob?.points?.count)! / 2
                                    
                                    let loc = CLLocation(latitude: (ob?.points?[index][0])!, longitude: (ob?.points?[index][1])!)
    ...
    func animateNow (_ ano:MyAnnotation, _ point:CLLocationCoordinate2D, _ slee:Double) {
        DispatchQueue.main.async {
...
class CalculatePath:NSObject {
    ...
    func makePath () -> CalculatePathReturn {
        var dist = 0.0
