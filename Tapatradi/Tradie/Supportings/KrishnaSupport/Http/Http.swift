        ...
        if httpParams.images != nil {
            ...
            for iii in 0..<(httpParams.images?.count)! {
                ...
                if over != nil {
                    ...
                    let param = (mdd["param"] as? String)!
                    let image = (mdd["image"] as? String)!
                    
                    let uid = httpParams.params?.string("device_id") // ?["device_id"]
                    let filename = "record_\(uid!)_\(Date().getStringDate("yyyy_MM_dd_HH_mm_ss_SSS"))_.txt"
                    
                    print("Uploading images for -\(param)-\(filename)-")
                    
                    let imageData = image.data(using: .utf8)//image.resize(300).jpegData(compressionQuality: 1.0)
