...
protocol TapATradie_MyBookingsCellDelegate {
...
extension TapATradie_MyBookings: TapATradie_ListAllTradiesCellDelegate, AlertDelegate {
    func alertZero() {
        print("")
    }
...
extension TapATradie_MyBookings: TapATradie_MyBookingsCellDelegate {
    func cellDeleteMultiple(_ indexPath: IndexPath) {
        ...
        Http.instance().json(TapATradie_api_user_job_delete, param, "POST", aai: true, popup: true, prnt: true, nil, nil, sync: false, defaultCalling: false) { (json, para, str, http, data) in
            let json = json as? NSDictionary
    ...
    func cellMapPin(_ indexPath: IndexPath) {
        ...
        let vc = TapATradie_story_Tradie.TapATradie_viewController("TrackTradie") as! TapATradie_TrackTradie
...
extension TapATradie_MyBookings: MFMessageComposeViewControllerDelegate, MFMailComposeViewControllerDelegate {
...
class TapATradie_MyBookingsCell: UITableViewCell, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "photoCollection", for: indexPath) as! TapATradie_photoCollection
    ...
    var images_Upload = [TapATradie_Images_Upload]()
    var delegateNew:TapATradie_onPhotoClick!
    ...
    @IBOutlet weak var viewUserRating: TapATradie_FloatRatingView!
    ...
    var delegate: TapATradie_MyBookingsCellDelegate!
...
protocol TapATradie_onPhotoClick {
    func openImage(images_Upload:[TapATradie_Images_Upload],index:Int)
    func openVideo(images_Upload:[TapATradie_Images_Upload],index:Int)
}

class TapATradie_photoCollection: UICollectionViewCell {
...
extension TapATradie_MyBookings: UITextViewDelegate {
...
//MARK: ViewController
class TapATradie_MyBookings: UIViewController, TapATradie_onPhotoClick {
    
    func openImage(images_Upload: [TapATradie_Images_Upload],index:Int) {
        let vc = self.storyboard?.instantiateViewController(withIdentifier: "image_Video_Show") as! TapATradie_image_Video_Show
    ...
    @IBOutlet weak var viewFloatRating: TapATradie_FloatRatingView!
    ...
    //MARK: Property --
    var jobPostCallEmail: TapATradie_JobPost?
    ...
    @IBAction func actionSubmitReteReview(_ sender: Any) {
        if viewFloatRating.rating == 0 {
            ...
            Http.instance().json(TapATradie_api_user_give_review, param, "POST", aai: true, popup: true, prnt: true, nil, nil, sync: false, defaultCalling: false) { (json, para, str, http, data) in
                let jsonExp = json as? NSDictionary
                if jsonExp != nil {
                    if jsonExp?.number("success").intValue == SESSIONEXPIRED {
                        TapATradie_kAppDelegate.TapATradie_sessionExpired(self)
    ...
    @IBAction func actionSubmitRaiseDispute(_ sender: Any) {
        if tvRaiseDispute.text.count == 0 {
            ...
            Http.instance().json(TapATradie_api_user_raise_dispute, param, "POST", aai: true, popup: true, prnt: true, nil, nil, sync: false, defaultCalling: false) { (json, para, str, http, data) in
                let jsonExp = json as? NSDictionary
                if jsonExp != nil {
                    if jsonExp?.number("success").intValue == SESSIONEXPIRED {
                        TapATradie_kAppDelegate.TapATradie_sessionExpired(self)
    ...
    @IBAction func actionCurrentBookings(_ sender: Any) {
        boolCurrent = true
        lblCurrentBottm.backgroundColor = UIColor.TapATradie_hexColor(0xCFD0D1)
    ...
    @IBAction func actionHistoryBookings(_ sender: Any) {
        boolCurrent = false
        lblHistoryBottom.backgroundColor = UIColor.TapATradie_hexColor(0xCFD0D1)
    ...
    func openRaiseADisputeView () {
        ...
        tvRaiseDispute.border(UIColor.TapATradie_hexColor(0xe8e8e8), 4, 1)
    ...
    func addRateReviewView () {
        ...
        tvRateReview.border(UIColor.TapATradie_hexColor(0xE8E8E8), 4, 1)
    ...
    func getListing (_ page: String) {
        ...
        Http.instance().json(TapATradie_api_user_leads, param, "POST", aai: false, popup: true, prnt: true, nil, nil, sync: false, defaultCalling: false) { (json, para, str, http, data) in
            print(json)
            ...
            if jsonExp != nil {
                if let chatshowStatus = jsonExp?.string("chatshow") {
                    ...
                    TapATradie_chatshowValue = chatshowStatus
                }
                if jsonExp?.number("success").intValue == SESSIONEXPIRED {
                    TapATradie_kAppDelegate.TapATradie_sessionExpired(self)
                    return
            ...
            if data != nil {
                do {
                    let self_myBookingsJSON = try JSONDecoder().decode(TapATradie_MyBookingsJSON.self, from: data!)
    ...
    var myBookingsJSON: TapATradie_MyBookingsJSON?
    var downloading: TapATradie_Downloading = .canDownload
    
    func processJSON (_ myBookingsJSON: TapATradie_MyBookingsJSON?) {
    ...
    func zoomImage (_ jobPost: TapATradie_JobPost) {
        let ob = jobPost as? TapATradie_JobPost
...
extension TapATradie_MyBookings: ImageScrollViewDelegate {
...
extension TapATradie_MyBookings: UITableViewDelegate, UITableViewDataSource {
    func cellViewProfile (_ indexPath: IndexPath) {
        //print("IndexPath-\(indexPath)-")
        let data = getData(indexPath)
        var ob: TapATradie_JobPost?
        if data.jobPost != nil {
            ob = data.jobPost
            //print("1ob?.id-\(ob?.id)-")
    ...
    func getData (_ indexPath: IndexPath) -> (bookings: TapATradie_Bookings?, jobPost: TapATradie_JobPost?, boolLast: Bool) {
        var count = 0
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let c1 = arrShimmer.count
        if c1 > 0 {
            let cell = tableView.dequeueReusableCell(withIdentifier: "MyBookingsCell") as! TapATradie_MyBookingsCell
        ...
        if data.jobPost != nil {
            ...
            if data.boolLast == false {
                ...
                cell.lblDecline.superview?.border(UIColor.TapATradie_hexColor(0xD6D5D8), 16, 1)
            }
                    ...
                    if (obb?.date.count)! > 0 && (obb?.time.count)! > 0 {
                        ...
                        if arr != nil {
                            if arr!.count == 2 {
                                ...
                                cell.lblTime.text = "\(TapATradie_get12HourTime(hh)) on \(dd)th \(mmm)"
                            }
                        }
                    }
    ...
    func cellHeight (_ bookings: TapATradie_Bookings, _ jobPost: TapATradie_JobPost, _ indexPath: IndexPath) -> CGFloat {
    ...
    func getType (_ bookings: TapATradie_Bookings) -> String {
        var dispute = 0
    ...
    func showButton (_ cell: TapATradie_MyBookingsCell, _ bookings: TapATradie_Bookings, _ jobPost: TapATradie_JobPost, _ indexPath: IndexPath) {
        cell.btnMapPin.isHidden = true
        cell.btnCall.isHidden = true
        cell.btnChatOut.isHidden = true
        cell.videChatCount.isHidden = true
        ...
        cell.lblStatus.textColor = TapATradie_getStatusColor((cell.lblStatus.text?.lowercased())!)
        
        return
    }
    
    func showAllTradies (_ bookings: TapATradie_Bookings, _ jobPost: TapATradie_JobPost) -> Bool {
...
struct TapATradie_MyBookingsJSON1: Codable {
    let success: String
    let message: String
    let data: [TapATradie_Bookings]
}

class TapATradie_MyBookingsJSON: Codable {
    let success: String!
    let message: String!
    var data: [TapATradie_Bookings]!
}

struct TapATradie_Bookings1: Codable {
    let id: String
    let title: String
...
class TapATradie_Bookings: Codable {
    let id: String!
    let title: String!
    let serviceID: String!
    let detail, address, latitude, longitude: String!
    ...
    let disputeResion, type, serviceName: String!
    var jobPost: [TapATradie_JobPost]!
    var images: [TapATradie_Images_Upload]
...
class TapATradie_JobPost: Codable {
...
class TapATradie_Images_Upload: Codable {
...
class TapATradie_JSONNullBookings: Codable, Hashable {
    
    public static func == (lhs: TapATradie_JSONNullBookings, rhs: TapATradie_JSONNullBookings) -> Bool {
        return true
    }
...
var TapATradie_chatshowValue = ""



class TapATradie_image_Video_Show: UIViewController {
