...
class TapATradie_MyProfile: UIViewController, TapATradie_DeleteAccount {
    ...
    override func viewDidLoad() {
        ...
        let profilePic = TapATradie_Key_User_profile_pic.TapATradie_getUserValue() as! String
        if profilePic.count > 0 {
            ...
            print("url-\(url)-")
            imgUser.TapATradie_downloaded(from: "\(url)",contentMode: .scaleToFill)
            print("First")
        } else {
            print("Second")
        ...
        tfmFullName.text = ((TapATradie_Key_User_full_name.TapATradie_getUserValue() as? String)?.capFirstLetter())!
        tfmEmail.text = (TapATradie_Key_User_email.TapATradie_getUserValue() as? String)!
        
        
        
        
        //tfmMobile.text = (Key_User_mobile.getUserValue() as? String)!
        tfmMobile.text = (TapATradie_Key_User_mobile_withCode.TapATradie_getUserValue() as? String)!
    ...
    @IBAction func actionDeleteAccount(_ sender: UIButton) {
        let vc = self.storyboard?.instantiateViewController(withIdentifier: "MyDeleteAccountVC") as! TapATradie_MyDeleteAccountVC
    ...
    func setFirstName () {
        lblName.text = ((TapATradie_Key_User_full_name.TapATradie_getUserValue() as? String))!
    }
    ...
    @IBAction func actionUpdateNow(_ sender: Any) {
        if validation () {
            ...
            Http.instance().json(TapATradie_api_user_update_profile, param, "POST", aai: true, popup: true, prnt: true, nil, nil, sync: false, defaultCalling: false) { (json, para, str, http, data) in
                ...
                if json != nil {
                    if json?.number("success").intValue == 1 {
                        TapATradie_Key_User_full_name.TapATradie_setUserValue(self.tfmFullName.textField.text!)
                        TapATradie_Key_User_email.TapATradie_setUserValue(self.tfmEmail.textField.text!)
                        TapATradie_Key_User_mobile.TapATradie_setUserValue(self.tfmMobile.textField.text!)
    ...
    func uploadProfilePicture (_ image1: UIImage) {
        ...
        Http.instance().json(TapATradie_api_upload_profile_picture, param, "POST", aai: true, popup: true, prnt: true, nil, image, sync: false, defaultCalling: false) { (json, para, str, http, data) in
            let jsonExp = json as? NSDictionary
            if jsonExp != nil {
                if jsonExp?.number("success").intValue == SESSIONEXPIRED {
                   TapATradie_kAppDelegate.TapATradie_sessionExpired(self)
            ...
            if json != nil {
                if json?.number("success").intValue == 1 {
                    ...
                    TapATradie_Key_User_profile_pic.TapATradie_setUserValue((json?.string("data"))!)
                }
                Http.alert("", json?.string("message"))
            }
        }
...
extension TapATradie_MyProfile: ImageScrollViewDelegate {
...
extension NSMutableData {
    func TapATradie_appendString(string: String) {
        let data = string.data(using: String.Encoding.utf8, allowLossyConversion: true)
        append(data!)
    }
}

extension TapATradie_MyProfile: UITableViewDelegate, UITableViewDataSource {
...
extension TapATradie_MyProfile: UITextFieldDelegate {
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if superV == tfmMobile {
            ...
            if textField.text!.count <= 1 && string.count == 0 {
                ...
                let bool = TapATradie_validEmail (str)
                //print(bool)
                if bool {
                    (textField.superview as? MaterialTextFieldView)?.status = .varified
                    (textField.superview as? MaterialTextFieldView)?.showNoError()
...
func TapATradie_validEmail(_ testStr: String) -> Bool {
...
extension TapATradie_MyProfile: UINavigationControllerDelegate, UIImagePickerControllerDelegate {
...
extension UIImageView {
    func TapATradie_downloaded(from url: URL, contentMode mode: ContentMode = .scaleAspectFit) {
        contentMode = mode
    ...
    func TapATradie_downloaded(from link: String, contentMode mode: ContentMode = .scaleAspectFit) {
        guard let url = URL(string: link) else { return }
        TapATradie_downloaded(from: url, contentMode: mode)
    }
}
