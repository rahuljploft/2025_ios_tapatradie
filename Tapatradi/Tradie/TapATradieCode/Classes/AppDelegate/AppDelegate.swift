...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        if  NetworkUtils.TapATradie_isNetworkReachable() == true {
             print("true")
         }else{
             sleep(200000000)
         }
        ...
        self.TapATradie_navigationController = UINavigationController(rootViewController: vc!)
        self.TapATradie_navigationController?.isNavigationBarHidden = true
        self.window?.rootViewController = self.TapATradie_navigationController
        self.window?.makeKeyAndVisible()
        
        notification (application, launchOptions)
        
        TapATradie_SocketIOManager.shared.create()
        ...
        TapATradie_getAppVersionFromServer ()
        
        Config.shared.appRole = "user"
        Config.shared.registerFonts()
        TapATradie_NavigationManager.shared.navigationController = self.TapATradie_navigationController
        
        self.TapATradie_SetupPushNotification(application: application)
        return true
    }
    ...
    func TapATradie_SetupPushNotification(application: UIApplication) -> () {
    ...
    func application(_ application: UIApplication, didRegisterForRemoteNotificationsWithDeviceToken deviceToken: Data) {
        ...
        TapATradie_firebase_token = "\(TapATradie_tokenString(deviceToken))"
        print(TapATradie_tokenString(deviceToken)) // this method will convert token "Data" to string formate
    }
    ...
    func notification (_ application: UIApplication, _ launchOptions: [UIApplication.LaunchOptionsKey: Any]?) {
        ...
        TapATradie_registerForPushNotifications (application)
        
        TapATradie_subscribeToFirebase (true)
        ...
        if let option = launchOptions {
            ...
            if dt != nil {
                ...
                TapATradie_openScreenOnNotification(dt!)
            }
        }
    }
    ...
    func applicationDidEnterBackground(_ application: UIApplication) {
        TapATradie_registerBackgroundTask(application)
    ...
    func applicationWillEnterForeground(_ application: UIApplication) {
        TapATradie_boolAppInBackground = false
        // Called as part of the transition from the background to the active state; here you can undo many of the changes made on entering the background.
        
                TapATradie_readAllBadge ()
    }
...
extension AppDelegate: UNUserNotificationCenterDelegate, MessagingDelegate {
    func TapATradie_readAllBadge () {
        ...
        Http.instance().json(TapATradie_api_read_badge_notification, param, "POST", aai: false, popup: false, prnt: true, nil, nil, sync: false, defaultCalling: false) { (json, paras, str, data) in
    ...
    func userNotificationCenter(_ center: UNUserNotificationCenter, didReceive response: UNNotificationResponse, withCompletionHandler completionHandler: @escaping () -> Void) {
        ...
        TapATradie_openScreenOnNotification(userInfo as NSDictionary)
        
        TapATradie_readNotification (userInfo as NSDictionary)
...
extension AppDelegate: AlertDelegate {
    ...
    func TapATradie_alertOne() {
        if let url = "itms://itunes.apple.com/us/app/tap-a-tradie/id1473400994".TapATradie_url {
        ...
        if let appVersion = Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String {
            if let double = Double(appVersion.replacingOccurrences(of: ".", with: "")) {
                ...
                if av > 0 {
                    ...
                    Http.instance().json(TapATradie_api_get_versions, param, "POST", aai: true, popup: true, prnt: true, nil, nil, sync: false, defaultCalling: false) { (json, paras, str, data) in
                        let jsonExp = json as? NSDictionary
                        /*
