...
public extension NSArray {
    public func TapATradie_toString () -> String {
        do {
    ...
    public func TapATradie_getMutable (_ maA: NSMutableArray?) -> NSMutableArray? {
        let array = self
        var maA = maA
        if maA == nil {
            maA = NSMutableArray ()
        ...
        for iii in 0..<array.count {
            if let val = array[iii] as? String {
                ...
                maA?.add(val.TapATradie_getMutable(nil)!)
    ...
    public func TapATradie_toString3 (_ caller: Bool = true) -> String {
        var str = "["
        for iii in 0..<self.count {
            if str.count == 1 {
                str = TapATradie_logincIf(str, iii)
            } else {
                str = TapATradie_logincElse(str, iii)
            }
        }
        if caller {
            str = "\(str)]".colon ()
    ...
    func TapATradie_logincElse(_ str: String, _ iii: Int) -> String {
        var str = str
        if let val = self[iii] as? String {
            ...
            str = "\(str),\(val.TapATradie_toString3(false))"
    ...
    func TapATradie_logincIf(_ str: String, _ iii: Int) -> String {
        var str = str
        if let val = self[iii] as? String {
            str = "\(str)\(val.colon ())"
        } else if let val = self[iii] as? Double {
