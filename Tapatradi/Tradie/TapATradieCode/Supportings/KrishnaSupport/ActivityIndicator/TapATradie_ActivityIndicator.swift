...
import Foundation
import UIKit
public class TapATradie_ActivityIndicator {
    var container: UIView?
    var loadingView: UIView?
    var activityIndicator: UIActivityIndicatorView?
    static let shared: TapATradie_ActivityIndicator = { TapATradie_ActivityIndicator () } ()
        ...
        DispatchQueue.main.async {
            self.lockQueue.sync {
                if self.noofrequest == 0 {
                    ...
                    self.container?.backgroundColor = UIColor.TapATradie_hexColor(0xffffff, alpha: 0.3)
                    self.loadingView?.frame = CGRect(x: 0, y: 0, width: 80, height: 80)
                    self.loadingView?.center = (self.appDelegate?.window??.center)!
                    self.loadingView?.backgroundColor = UIColor.TapATradie_hexColor(0x444444, alpha: 0.7)
