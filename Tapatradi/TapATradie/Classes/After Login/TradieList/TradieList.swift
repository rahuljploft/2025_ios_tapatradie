...
protocol TradieListCellDelegate {
...
class TradieListCell: UITableViewCell {
    ...
    var delegate: TradieListCellDelegate!
...
extension TradieList: TradieListCellDelegate {
...
//        }
        ...
        let cell = tblTradies.dequeueReusableCell(withIdentifier: "TradieListCell", for: indexPath) as! TradieListCell
        
        let cardView = cell.cardView
        cardView?.layer.backgroundColor = UIColor.clear.cgColor
        cardView?.layer.borderColor = UIColor.hexColor(0xDFDFDF).cgColor
...
//            cell.subtitleViewC.layer.borderColor = UIColor.clear.cgColor
    ...
    func forServiceCalled (_ ob: Tradies) -> String? {
...
extension UIButton {
    func selectedFilter (_ bool: Bool) {
        if bool {
            self.setTitleColor(UIColor.hexColor(0xF7941D), for: .normal)
            self.layer.borderColor = UIColor.hexColor(0xF7941D).cgColor
        } else {
            self.setTitleColor(UIColor.hexColor(0x707070), for: .normal)
            self.layer.borderColor = UIColor.hexColor(0x000000).withAlphaComponent(0.2).cgColor
        }
    }
}
...
class TradieList: UIViewController {
...
//                }
    ...
    func selectedFilter () {
        btnFilterLowToHigh.setTitleColor(UIColor.hexColor(0x707070), for: .normal)
        btnFilterLowToHigh.layer.borderColor = UIColor.hexColor(0x000000).withAlphaComponent(0.2).cgColor
        
        btnFilterHighToLow.setTitleColor(UIColor.hexColor(0x707070), for: .normal)
        btnFilterHighToLow.layer.borderColor = UIColor.hexColor(0x000000).withAlphaComponent(0.2).cgColor
        
        if filter == Filter.lowtohigh {
            btnFilterLowToHigh.setTitleColor(UIColor.hexColor(0xF7941D), for: .normal)
            btnFilterLowToHigh.layer.borderColor = UIColor.hexColor(0xF7941D).cgColor
        } else if filter == Filter.hightolow {
            btnFilterHighToLow.setTitleColor(UIColor.hexColor(0xF7941D), for: .normal)
            btnFilterHighToLow.layer.borderColor = UIColor.hexColor(0xF7941D).cgColor
        }
    }
    ...
    @IBAction func actionLowToHigh(_ sender: Any) {
        filter = Filter.lowtohigh
        
        selectedFilter ()
    }
    
    @IBAction func actionHighToLow(_ sender: Any) {
        filter = Filter.hightolow
...
extension TradieList: ImageScrollViewDelegate {
...
extension TradieList: UITableViewDelegate, UITableViewDataSource {
...
//        }
    ...
    func bubbleSort(_ ar: [Tradies]) -> [Tradies] {
        var ar = ar
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if c1 > 0 {
            let cell = tableView.dequeueReusableCell(withIdentifier: "TradieListCell") as! TradieListCell
...
//                cell.subtitleViewC.layer.borderColor = UIColor.clear.cgColor
        ...
        cell.imgCell.sd_setImage(with: url.url) { (img, err, ty, ul) in
            if img == nil {
...
extension String {
    ...
    func checkApprox (_ dict: NSDictionary?) -> String {
        if dict == nil {
            return self
        }
