...
protocol ListAllTradiesCellDelegate {
...
class ListAllTradiesCell: UITableViewCell {
    ...
    var delegate: ListAllTradiesCellDelegate!
...
extension ListAllTradies: ListAllTradiesCellDelegate {
    func chatList(_ indexPath: IndexPath) {
...
class ListAllTradies: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        tblAllTradies.border(UIColor.hexColor(0xD6D5D8), 16, 1)
                    ...
                    if (bookings?.date.count)! > 0 && (bookings?.time.count)! > 0 {
                        ...
                        if arr != nil {
                            if arr!.count == 2 {
                                ...
                                lblTimeService.text = "\(get12HourTime(hh)) on \(dd)th \(mmm)"
                            }
                        }
                    }
                    
                    lblDeclined.textColor = getStatusColor((lblDeclined.text?.lowercased())!)
                }
            }
        }
    }
...
extension ListAllTradies: ImageScrollViewDelegate {
