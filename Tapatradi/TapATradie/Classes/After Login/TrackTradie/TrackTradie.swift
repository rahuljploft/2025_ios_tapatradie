...
class TrackTradie: UIViewController {
    @IBAction func actionBack(_ sender: Any) {
        SocketIOManager.shared.sendRequestForLocationStop((jobPost?.providerID)!)
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        obMoveAnnotation = MoveAnnotation(mapTrackTradie)
        obMoveAnnotation.start()
        SocketIOManager.shared.vcTrackTradie = self
        mapTrackTradie.delegate = self
        urlTradiePicture = "\(Server)profile/\((jobPost?.providerID)!)/\((jobPost?.profilePic)!)"
        sendRequestForLocation ()
    }
    
    override func viewDidDisappear(_ animated: Bool) {
        SocketIOManager.shared.vcTrackTradie = nil
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if bookings != nil {
            ...
            if lat != nil && lng != nil {
                ...
                let ano = MyAnnotation(title: bookings!.address, coordinate: loc, isDriver:true, id:"")
        ...
        if jobPost != nil {
            ...
            if lat != nil && lng != nil {
                ...
                let ano = MyAnnotation(title: (jobPost?.fullName)!, coordinate: loc, isDriver:true, id:"")
                ano.uiimage = #imageLiteral(resourceName: "Group 3563-1")
                mapTrackTradie.addAnnotation(ano)
            }
        }
    ...
    func setReceiverImageAndMove (_ image: UIImage, _ lat: CLLocationDegrees, _ lng: CLLocationDegrees) {
        ...
        if obMoveAnnotation != nil {
            if obMoveAnnotation.ano == nil {
                let ano = MyAnnotation(title: (jobPost?.fullName)!.capFirstLetter(), coordinate: loc, isDriver:true, id:"")
                ano.uiimage = image
    ...
    func showLocation (_ lat: CLLocationDegrees, _ lng: CLLocationDegrees, _ id: String) {
        ...
        if bookings?.serviceName != nil {
            if (bookings?.serviceName)!.count > 0 {
                img = getMapPin((bookings?.serviceName)!)
            }
        }
    ...
    func sendRequestForLocation () {
        if self.boolGettingReceiverLocation == false {
            SocketIOManager.shared.sendRequestForLocationStart((jobPost?.providerID)!)
        }
...
extension TrackTradie: MKMapViewDelegate {
    func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
        ...
        if img != nil {
            anView!.image = img?.roundedRectImageFromImage(image: img!, imageSize: sz, cornerRadius: rds/2)
        }
