...
class MyChatScreen_VC: UIViewController, UITableViewDelegate, UITableViewDataSource, OpenMediaFile, UIImagePickerControllerDelegate & UINavigationControllerDelegate,UIDocumentMenuDelegate,UIDocumentPickerDelegate {
    ...
    override func viewDidLoad() {
        ...
        let image = "\(Server)profile/\(providerId)/\(profileURl)"
        print(image)
...
extension MyChatScreen_VC {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        print("Index - \(indexPath.row) : \(Server)profile/\(data?.senderID ?? 0)/\(data?.senderprofile ?? "")")
        
        if "\(data?.senderID ?? 0)" == userID && "\(data?.type ?? "")" == "image" {
            let cell = tableView.dequeueReusableCell(withIdentifier: "SendImageMessageCell_TVC") as! SendImageMessageCell_TVC
            cell.updateCell()
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: "ReciverImageMessageCell_TVC") as! ReciverImageMessageCell_TVC
            cell.updateCell()
            cell.selectionStyle = .none
            cell.lbl_date.text = timeStampToDate(dateVal: "\(data?.createdOn ?? "")")
            //cell.lbl_date.text = ""
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: "SendTextMessageCell_TVC") as! SendTextMessageCell_TVC
            cell.updateCell()
            cell.selectionStyle = .none
            cell.lbl_date.text = timeStampToDate(dateVal: "\(data?.createdOn ?? "")")
            cell.lbl_SenderName.text = "\(data?.senderName ?? "")"
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: "ReciveTextMessageCell_TVC") as! ReciveTextMessageCell_TVC
            cell.updateCell()
            cell.selectionStyle = .none
