...
class EnterYourDetail: UIViewController {
    ...
    @IBAction func actionGetStarted(_ sender: Any) {
        ...
        if tfvFullName.status == .varified {
            ...
            Http.instance().json(api_user_register_step2, param, "POST", aai: true, popup: true, prnt: true, nil, nil, sync: false, defaultCalling: false) { (json, paras, str, data) in
                ...
                if let json = json as? NSDictionary {
                    if json.number("success").intValue == 1 {
                        ...
                        let vc = story_Auth.viewController("SeeTradiesArround")
