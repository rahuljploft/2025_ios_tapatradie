...
func getCityStateCountry (_ dt: NSDictionary) -> (city: String, state: String, country: String) {
    var city = ""
    var state = ""
    var country = ""
    
    if let address_components = dt["address_components"] as? NSArray {
        for i in 0..<address_components.count {
            if let dtt = address_components[i] as? NSDictionary {
                if let types = dtt["types"] as? NSArray {
                    for j in 0..<types.count {
                        if let str = types[j] as? String {
                            if str == "administrative_area_level_2" {
                                city = dtt.string("long_name")
                            } else if str == "country" {
                                country = dtt.string("long_name")
                            } else if str == "administrative_area_level_1" {
                                state = dtt.string("long_name")
                            }
                        }
                    }
                }
...
func returnAddressLatLong(_ lat: NSNumber?, _ long: NSNumber?,
    ...
    if lat != nil && long != nil {
        if let dtt = results[iii] as? NSDictionary {
            let csc = getCityStateCountry(dtt)
            addressObject.city = csc.city
            addressObject.country = csc.country
            addressObject.state = csc.state
